...
export default {
  ...
  watch: {
    value: {
      ...
      handler(newValue, oldValue) {
        let newValueClone = []; let oldValueClone = []
  ...
  methods: {
    AddOptionClick() {
      ...
      this.$refs.selectTree.setSoftFocus()
    },
    mapDataById(data) {
      data.forEach(item => {
    ...
    selectClearAll() {
      ...
      this.$emit('clear')
      if (this.showCheckbox) {
        this.$emit('node-check', this.getCheckedNodes())
      }
      if (!this.multiple) {
    ...
    selectRemoveTag(tag) {
      ...
      if (this.checkStrictly) {
        ...
        // 获取过滤的子级id
        const getChildrenIds = function(data) {
          let ids = []
          data.forEach(item => {
        ...
        // 获取过滤的父级id
        const getParentIds = function(data, pid) {
          let ids = []
          for (const item of data) {
    ...
    treeNodeClick(data, node, component) {
      if (this.showCheckbox) { // 多选
        ...
        if (this.onlyFinal && !node.isLeaf) {
          return
        } else if (this.structure && node.isLeaf && data.structure === 'agency') {
          return
        }
    ...
    renderContent(h, { node, data, store }) {
      if (this.nodeFormatter) {
        return this.nodeFormatter({ node, data, store })
      } else {
        if (node.isLeaf) {
          return (
    ...
    treeCheck(data, { checkedNodes, checkedKeys, halfCheckedNodes, halfCheckedKeys }) {
      if (this.checkStrictly) { // 父子不相关联
        ...
        let currendChidenIds = [] // 当前节点子节点id数组
        //  const checkedLabelArray = [] // 当前树的选择节点名称
        // 获取子树id
        const getIds = array => {
          let checkedIds = []
